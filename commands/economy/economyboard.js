...
module.exports = {
  ...
  async execute(message, args, client) {
    const entries = getEconomyLeaderboard(message.guild.id);
    ...
    const medals = ['🥇', '🥈', '🥉'];
    const leaderboard = entries.map((e, i) => `${medals[i] || `**${i + 1}.**`} <@${e.userId}> — **${((e.wallet || 0) + (e.bank || 0)).toLocaleString()}** coins`).join('\n');
