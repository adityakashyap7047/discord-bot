...
module.exports = {
  ...
  async execute(source, arg2, arg3) {
    ...
    if (!amountInput) {
      return source.reply({ content: '❌ Please specify an amount to deposit.', ephemeral: true }).catch(() => {});
    }
    ...
    if (amount > data.wallet) {
      return source.reply({
        content: `❌ You only have **${data.wallet.toLocaleString()}** coins in your wallet.`,
        ephemeral: true,
      }).catch(() => {});
    }
